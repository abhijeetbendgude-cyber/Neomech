...
const Navbar = () => {
  ...
  return (
    <nav className={`w-full fixed top-0 left-0 z-50 transition-all duration-300 ${isScrolled ? 'shadow-lg' : ''}`}>
      {/* Top Bar */}
      <div className={`w-full bg-white border-b border-gray-200 transition-all duration-300 ${isScrolled ? 'h-0 overflow-hidden md:h-[40px]' : 'h-0 md:h-[40px]'}`}>
          ...
          <div className="flex justify-end items-center h-full gap-4 overflow-x-auto hide-scrollbar">
            {topLinks.map((link, index) => (
              <div key={link.name} className="flex items-center shrink-0">
                <Link to={link.to} className="text-[13px] text-[#1a4789] hover:text-[#003087] whitespace-nowrap transition-all duration-200 hover:scale-105 transform">
                  {link.name}
                </Link>
      ...
      {/* Main Navigation Bar */}
      <div className={`w-full bg-gradient-to-r from-[#003087] to-[#002674] transition-all duration-300 ${isScrolled ? 'bg-opacity-95 backdrop-blur-sm' : ''}`}>
          ...
          <div className="flex-1">
            <div className="container mx-auto px-4 lg:px-8">
              <div className="flex items-center justify-end h-full">
                ...
                {/* Mobile Buttons (Left & Right) */}
                <div className={`lg:hidden fixed inset-0 top-[60px] bg-[#002674] transform transition-all duration-300 ${isOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}>
                  ...
                  <button
                    ...
                    className="text-white p-3 transition-all duration-300 hover:scale-110 active:scale-95"
        ...
        {/* Mobile Main Menu */}
        {isMainMenuOpen && (
          <div className="lg:hidden fixed inset-0 top-[60px] bg-[#002674] z-40">
            <div className="container mx-auto px-4 py-6 space-y-2 overflow-y-auto h-full pb-20">
              <div className="border-b border-gray-700 pb-2 mb-4">
                <button
                  ...
                  className="w-full flex justify-between items-center py-4 text-white text-base font-medium hover:bg-[#001f5c] px-4 rounded-lg transition-all duration-300 active:scale-95"
                >
                  <span>Verticals</span>
                  <svg className={`w-6 h-6 transform transition-transform duration-300 ${isVerticalsOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
              ...
              {mainLinks.map((link) => (
                <Link
                  ...
                  className="block py-3 text-white text-base font-medium hover:bg-[#001f5c] px-4 rounded-lg transition-all duration-300 hover:translate-x-2 transform active:scale-95"
                >
                  {link.name}
                </Link>
              ))}
        ...
        {/* Mobile Top Menu */}
        {isTopMenuOpen && (
          <div className="lg:hidden fixed inset-0 top-[60px] bg-[#001f5c] z-40">
            <div className="container mx-auto px-4 py-6 space-y-2 overflow-y-auto h-full pb-20">
              {topLinks.map((link) => (
                <Link
                  key={link.name}
                  ...
                  className="block py-2 text-white/80 text-sm hover:bg-[#003087] px-4 rounded-lg transition-all duration-300 hover:translate-x-2 transform active:scale-95"
                >
                  {link.name}
                </Link>
              ))}
